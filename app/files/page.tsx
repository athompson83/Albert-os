            ...
            <thead>
              <tr style={{ borderBottom: '1px solid var(--border)' }}>
                {['Name', 'Size', 'Modified'].map(h => (
                  <th key={h} style={{ padding: '12px 8px', textAlign: 'left', fontSize: 12, fontWeight: 600, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
                ...
                <tr key={f.name} style={{ borderBottom: i < files.length - 1 ? '1px solid var(--border)' : 'none' }}>
                  <td style={{ padding: '12px 10px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      ...
                      <span style={{ fontSize: 14, color: '#e5e5e5', overflowWrap: 'anywhere' }}>{f.name}</span>
                    </div>
                  </td>
                  <td style={{ padding: '12px 8px', fontSize: 13, color: 'var(--text-muted)' }}>{f.size}</td>
                  <td style={{ padding: '12px 8px', fontSize: 13, color: 'var(--text-muted)', overflowWrap: 'anywhere' }}>{f.modified}</td>
                </tr>
              ))}
            </tbody>
          </table>
